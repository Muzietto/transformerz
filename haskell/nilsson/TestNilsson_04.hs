...
module TestNilsson_04 where 
  ...
  testList01 :: Test
  testList01 = 
      TestCase $ assertEqual "simple test list 01"
                             ((L "qwerty") >>= (\x -> L $ x ++ x)) (L "qwertyqwerty")
...
------------- RT + ET + WT + ST -----------------
  ...
  testEval5dLiteral = 
      TestCase $ assertEqual "eval5d should evaluate a silly literal while updating state and filling the log"
                             ((Just (IntVal 18), ["literal"]), 1) (runEval5d Map.empty 0 $ eval5d (Lit 18))

  testEval5dSimpleApp :: Test
  testEval5dSimpleApp = 
      TestCase $ assertEqual "eval5d should make a simple application while updating state and filling the log"
  ...
  testEval5dComplexApp = 
      TestCase $ assertEqual "eval5d should make a complex application while updating state and filling the log"
  ...
  testEval5dCurriedApp = 
      TestCase $ assertEqual "eval5d should make a partial application while updating state and filling the log"
  ...
  testEval5dCurriedApp2 = 
      TestCase $ assertEqual "eval5d should spit Nothing in case of errors while updating state and filling the log"
  ...
  testEval5dWatIsXPlusY = 
      TestCase $ assertEqual "eval5d should sum two vars while updating state and filling the log"
                             ((Just (IntVal 357), ["sum",
                                                  "lookup xxxx",
                                                  "lookup ok",
                                                  "lookup yyyy",
  ...
  testEval5dWatIsXPlusCrash = 
      TestCase $ assertEqual "eval5d should fail summing stuff when one ain't IntVal while updating state and filling the log"
                             ((Nothing, ["sum",
                                       "literal",
                                       "lambda",
                                       "sum ko"
  ...
  testEval5dVarUndefined = 
      TestCase $ assertEqual "eval5d should fail on non-existing vars and write about it while updating state and filling the log"
        ((Nothing, ["lookup zzzz","lookup ko"]), 1)
        (runEval5d two_vars_env 0 (eval5d (Var "zzzz")))

  testEval5dVarXxxx :: Test
  testEval5dVarXxxx = 
      TestCase $ assertEqual "eval5d should lookup a Var and write about it while updating state and filling the log"
